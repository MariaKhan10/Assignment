...
# Encapsulation:
# This class encapsulates data (target_lang, lang_map) and method (translate) in one unit.
class Translator:
...
# Abstraction:
# This abstract base class hides implementation details and provides a simple interface.
...
# Polymorphism:
# The same method get_input behaves differently based on the class implementation.
class InputHandler:
    def get_input(self):
        raise NotImplementedError()
...
# Supporting class — not directly tied to a specific principle, but used for functionality
class PDFExporter:
...
st.set_page_config(page_title="Royal MediCare – By Maria", layout="centered")
...
.subtitle {
    font-size: 1.1rem;
    text-align: center;
    color: #6A1B9A; /* Brighter color for better visibility */
    margin-bottom: 2rem; /* Added more spacing below subtitle */
...
.emoji-title {
    font-size: 2rem;
    text-align: center;
    margin-top: 1rem; /* Increased spacing from description */
    color: #6A1B9A; /* Brighter color for better contrast */
}
</style>""", unsafe_allow_html=True)

st.markdown('<div class="main-title">SehatBot 🩺</div>', unsafe_allow_html=True)  
st.markdown('<div class="subtitle">AI-Powered Instant Medical Help — Created by <b>Maria Khan</b> 💜</div>', unsafe_allow_html=True)
...
st.markdown('<div class="emoji-title">Get Help Anytime, Anywhere! 💬</div>', unsafe_allow_html=True)


# -------------------- Sidebar --------------------
st.sidebar.image("assets/image4.gif", use_container_width=True)
lang = st.sidebar.selectbox("🌐 Choose language", ["English", "Urdu", "Hindi"])
...
if st.button("🩺 Get Advice"):
    if symptoms.strip():
        ...
        with st.spinner("🩺 Hang tight! We're scanning your symptoms...\n⏳ Finding diagnosis and remedies..."):
...
if st.session_state.get("final_result"):
    ...
    # Check if follow-up response exists
...
if st.session_state.get("final_result"):
    ...
    if not st.session_state["history"] or st.session_state["history"][-1]["result"] != new_entry["result"]:
        st.session_state["history"].append(new_entry)

if st.sidebar.checkbox("📚 Show Past Reports"):
    st.markdown("## 🕘 History")
    for record in reversed(st.session_state["history"]):
        st.markdown(f"**{record['timestamp']}** - {record['symptoms']}")
        st.code(record['result'])
        st.markdown("---")
